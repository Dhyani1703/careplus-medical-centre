import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";

function CompletedAppointments() {
  const navigate = useNavigate();
  const [appointments, setAppointments] = useState([]);

  useEffect(() => {
    try {
      const saved = JSON.parse(
        localStorage.getItem("careplusAppointments") || "[]"
      );
      setAppointments(
        saved.filter(
          (item) =>
            item.appointmentStatus === "completed" &&
            item.paymentStatus === "paid"
        )
      );
    } catch {
      setAppointments([]);
    }
  }, []);

  return (
    <div className="records-page">
      <div className="records-page-header">
        <div>
          <h1>Completed Appointments</h1>
          <p>View paid appointments and reopen saved receipts.</p>
        </div>
        <button
          type="button"
          className="back-dashboard-btn"
          onClick={() => navigate("/dashboard")}
        >
          ← Back to Dashboard
        </button>
      </div>

      {appointments.length === 0 ? (
        <div className="records-empty-state">
          <span>🧾</span>
          <h2>No completed appointments</h2>
          <p>Your paid appointments and receipts will appear here.</p>
        </div>
      ) : (
        <div className="records-grid">
          {appointments.map((appointment) => (
            <article className="record-card" key={appointment.id}>
              <div className="record-card-header">
                <div>
                  <h2>{appointment.doctorName}</h2>
                  <p>{appointment.specialty}</p>
                </div>
                <span className="record-status completed">Completed</span>
              </div>

              <div className="record-details">
                <p><strong>Date:</strong> {appointment.formattedDate}</p>
                <p><strong>Time:</strong> {appointment.time}</p>
                <p><strong>Amount Paid:</strong> ${Number(appointment.fee).toFixed(2)}</p>
                <p>
                  <strong>Receipt:</strong>{" "}
                  {appointment.receiptNumber ||
                    `CP-${String(appointment.id).slice(-8)}`}
                </p>
              </div>

              <button
                type="button"
                className="record-primary-btn"
                onClick={() => navigate(`/receipt/${appointment.id}`)}
              >
                View Receipt
              </button>
            </article>
          ))}
        </div>
      )}
    </div>
  );
}

export default CompletedAppointments;